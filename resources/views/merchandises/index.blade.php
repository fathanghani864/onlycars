...
@section('content')
    <div class="min-h-screen flex flex-col bg-gray-900 p-10">
        <div class="bg-gray-800 shadow-2xl rounded-2xl p-12 max-w-7xl mx-auto w-full">
            <h1 class="text-4xl font-extrabold mb-8 text-center text-white border-b-2 border-gray-700 pb-6">
                🗓️ Daftar Event
            </h1>

            @if(session('success'))
                <div class="bg-green-600 text-white px-6 py-4 rounded-lg relative mb-8 shadow-md" role="alert">
                    <span class="block sm:inline font-semibold">{{ session('success') }}</span>
                </div>
            @endif

            <div class="flex justify-between items-center mb-8">
                <p class="text-gray-400">Total Event: {{ $events->count() }}</p>
            ...
            @if($events->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($events as $event)
                        <div
                            class="bg-gray-700 rounded-xl shadow-lg overflow-hidden border border-gray-600 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                            {{-- Mengatur kontainer gambar dengan tinggi lebih ramping dan meletakkan gambar di tengah --}}
                            <div class="w-full h-72 flex items-center justify-center overflow-hidden rounded-t-xl">
                            ...
                            <div class="p-6 flex flex-col items-center text-center">
                                ...
                                <p class="text-sm text-gray-400 mt-2">{{ $event->location }}</p>
